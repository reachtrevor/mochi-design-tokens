...
async function main() {
    ...
    program
        ...
        .option("-o, --out <directory>", "Output directory for generated CSS files", DEFAULT_OUTPUT_DIR)
...
async function run(options) {
    ...
    try {
        ...
        const totalFilesToProcess = discoveredFiles.outputFiles.length + discoveredFiles.themeFiles.length;
        if (totalFilesToProcess === 0) {
            logger.warn("No files to transform. Exiting.");
            return;
        }
        // Step 3: Transform tokens to CSS
        const { results, stats } = await transformTokens(discoveredFiles, options.outputDir);
        // Step 4: Print summary
        logger.newline();
        logger.summary({
            referenceFiles: discoveredFiles.referenceFiles.map((f) => getRelativePath(f, tempDir)),
            inputFiles: [
        ...
        // Success message
        logger.success(`Generated ${results.length} CSS file(s) in ${options.outputDir}`);
    }
    catch (error) {
        ...
        if (error instanceof Error) {
            ...
            if (error.message.includes("not found")) {
                ...
                logger.info("Hint: Token files must have .inp.json or .ref.inp.json extension");
            }
