 ...
 */
function findFilesRecursive(dir: string, files: string[] = []): string[] {
 ...
 */
export function discoverTokenFiles(extractedDir: string): DiscoveredFiles {
  ...
  const totalTokenFiles =
    referenceFiles.length + themeFiles.length + outputFiles.length;
  if (totalTokenFiles === 0) {
    throw new Error(
      "No token files found. Token files must have .inp.json, .theme.inp.json, or .ref.inp.json extension."
    );
  }
  ...
  if (totalOutputFiles === 0) {
    logger.warn(
      "No output files found. Only reference files (*.ref.inp.json) were discovered."
    );
  }

  // Log discovery results
  logger.info(
    `Found ${totalTokenFiles} token files (${referenceFiles.length} reference-only, ${themeFiles.length} themes, ${outputFiles.length} standard)`
  );
 ...
 */
export function getThemeName(inputPath: string): string {
  const basename = path.basename(inputPath);
 ...
 */
export function isThemeFile(inputPath: string): boolean {
